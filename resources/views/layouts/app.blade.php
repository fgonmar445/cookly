...
<body class="bg-slate-50 h-full text-slate-900">
    ...
    <div class="flex h-full overflow-hidden relative">
        ...
        <!-- MOBILE OVERLAY -->
        <div id="sidebar-overlay" onclick="toggleSidebar()" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-30 hidden lg:hidden transition-opacity duration-300 opacity-0"></div>

        <!-- SIDEBAR -->
        <aside id="sidebar" class="fixed inset-y-0 left-0 w-72 bg-white border-r border-slate-200 flex flex-col h-full z-40 transition-transform duration-300 -translate-x-full lg:translate-x-0 lg:static lg:inset-auto">
            ...
            <div class="p-8 pb-4 flex items-center justify-between">
                ...
                <button onclick="toggleSidebar()" class="lg:hidden p-2 text-slate-400 hover:text-slate-600 transition-colors">
            ...
            @auth
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('admin.ingredientes.index') }}" class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('admin.ingredientes.*') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50 hover:text-emerald-600' }}">
                            <svg class="w-5 h-5 mr-3 {{ request()->routeIs('admin.ingredientes.*') ? 'text-emerald-600' : 'text-slate-400 group-hover:text-emerald-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 border-t border-slate-100 bg-slate-50/50">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('profile.edit') }}" class="flex-1 flex items-center justify-center px-3 py-2 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg hover:bg-emerald-100 hover:border-emerald-200 transition-all shadow-sm text-center">
                        Perfil
                    </a>
        ...
        <div class="flex-1 flex flex-col min-w-0 h-full relative overflow-hidden">
            ...
            <!-- Header bar -->
            <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-10">
                <div class="flex items-center gap-4">
                    <!-- Mobile Hamburger -->
                    <button onclick="toggleSidebar()" class="lg:hidden p-2 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </button>
                    <h2 class="text-lg font-semibold text-slate-800 truncate max-w-[200px] md:max-w-none">
                        @yield('header_title', 'Bienvenido a Cookly')
                    </h2>
                </div>
                <div class="flex items-center gap-2 md:gap-4 text-slate-500">
                    <span class="text-xs md:text-sm hidden sm:inline">{{ now()->translatedFormat('d \d\e F, Y') }}</span>
